...
define( 'KOTOTSUGI_VERSION', '0.3.0' );
...
function kototsugi_import_remote_image( WP_REST_Request $request ) {
	...
	if ( $post_id && ! current_user_can( 'edit_post', $post_id ) ) {
		return new WP_Error(
			...
			__( 'You cannot attach images to this post.', 'kototsugi' ),
			array( 'status' => 403 )
		);
	}
	...
	$existing_ids = get_posts(
		array(
			...
			'meta_key'               => '_kototsugi_source_url',
			'meta_value'             => $url,
		)
	);
	...
	if ( ! $tmp_file ) {
		return new WP_Error(
			...
			__( 'A temporary file could not be created.', 'kototsugi' ),
			array( 'status' => 500 )
		);
	}
